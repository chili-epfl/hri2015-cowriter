...
tutors~\cite{Leyzberg2012} that can be adapted to each
learner~\cite{Leyzberg2014}.
...
experienced by the human tutor itself.  Our work concentrates on this latter
aspect: by demonstrating handwriting to a robot, we aim at improving the
\emph{child} performances. Note that our work must be distinguished from
traditional ``learning from demonstration'' approaches, as the agent we present
is only simulating fine motor skills for interaction purposes.
