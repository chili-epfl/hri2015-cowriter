...
demonstrated to participants~\cite{Berninger1997}. Furthermore, when compared
to screen-based agents, robotic partners have been shown in some contexts to 
increase users' compliance with tasks~\cite{Bainbridge2011}, maintain more 
effective long-term relationships~\cite{Kidd2008}, and produce greater 
learning gains when acting as tutors~\cite{Leyzberg2012}: benefits which we
hope will transfer into this application.

Robots have been used as teachers or social partners to promote children's
learning in a range of contexts, most commonly langauge skills~\cite{Han2010}, 
but also applications such as logic puzzles~\cite{Leyzberg2014}, in music 
classes~\cite{Han2009}, and physical skills such as caligraphy \cite{Matsui2013}.
Werfel suggests in~\cite{Werfel2014} that, until now, most of the work 
investigating how humans can instead teach robots (language \cite{Saunders2010} or 
table tennis \cite{Mulling2013}, for example) focuses on the robot's
benefits rather than the learning experienced by the human tutor. Our
work concentrates on this latter aspect. We note that our work differs from those
where a robot develops physical skills through `learning 
from demonstration' from humans, as the agent we present is only simulating 
fine motor skills for interaction purposes.

A robotic learning agent which 
employs the learning by teaching paradigm has 
...
proposing a robotic partner which is fully autonomous in its learning,
and second, by investigating the context
...
requires the supervision of an experimenter. The interaction finite state machine would
...
in order to alleviate the necessity of a conversational agent, we find reason to 
...
self-efficacy as when they are criticised in a typical educational context. 
